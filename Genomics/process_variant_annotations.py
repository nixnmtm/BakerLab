...
def get_causal_variants(df, af_cut="0.001"):
    """
    ...
    causal_variants = pd.concat([non_splice_vaiants, splice_variants]).reset_index()
    causal_variants.sort_values(by="Chr", key=lambda x: np.argsort(index_natsorted(causal_variants["Chr"])))
    
    causal_variants.set_index("Chr", inplace=True)
...
def populate_gene_details(sample_id, absolute_annovar_path):
    """
    **Populates gene deatils per variant**
    ...
    af_cut = 0.001
    variants = get_causal_variants(vc,str(af_cut))
    ...
    try:
        ...
        merged.to_csv(path.join(outpath, f"af_{af_cut}_imp_annotations.csv"), index=False)
        return merged
